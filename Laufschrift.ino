...
const unsigned char disp [NUMCHARS][8]={
...
void Write_Max7219_byte(unsigned char DATA)
{
  ...
  {     
    ...
    digitalWrite(dataIn,DATA&0x80);//Obtain the MSB of the data
    DATA = DATA<<1;
...
{ 
  ...
  {
    thischar=strchr(chars,text[j])-chars; // Index des Buchstaben im disp Array
    // Index des nächsten Buchstaben im disp Array
    ...
    for (k=0;k<8;k++)
    {
      for (i=0;i<8;i++)
      {
        data[i]=((unsigned int)disp[thischar][i]<<8 | disp[nextchar][i]) >>8-k;
      }
